...
// Simple password-protected admin area. Set ADMIN_USER / ADMIN_PASSWORD in your env.
function requireAdmin(req, res, next) {
  const expectedUser = process.env.ADMIN_USER || 'admin';
  const expectedPass = process.env.ADMIN_PASSWORD;
  if (!expectedPass) {
    return res.status(500).send('Admin area is not configured. Set ADMIN_PASSWORD in your environment.');
  }
